var annual_energy_costs = function annual_energy_costs(div) {
    ...
    d3.tsv("/www.retrofitanalysis.org/data/emissions.csv", function(error, data) {
        color.domain(["electrical_normalised_cost", "gas_normalised_cost"]);
        ...
        y.domain([0, 5 + d3.max(data, function(d) { return d.total; })]).nice();
        ...
        var property = svg.selectAll(".property")
            ...
            .on("mousemove", function() {
                return tooltip.style("top", (d3.event.pageY-10)+"px").style("left", (d3.event.pageX+10)+"px").html('Electrical Cost £' + numberFormat(d3.event.currentTarget.__data__.electrical_normalised_cost) + '<br />Gas Cost £' + numberFormat(d3.event.currentTarget.__data__.gas_normalised_cost) + '<br />Total Cost £' + numberFormat(d3.event.currentTarget.__data__.normalised_cost) )
                // .style("height", "38px");
            })
        ...
        legend.append("text")
            ...
            .text(function(d) {
                console.log(d);
                return d == 'electrical_normalised_cost'  ? 'Electrical Cost' :
                    d == 'gas_normalised_cost'  ?  'Gas Cost' :
                    'Unexpected Value';
            });
